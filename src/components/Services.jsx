...
export default function Services({ data, setData, openAdd, registerAdd }) {
  ...
  const openModal = (editing) => {
    setModal({
      editing,
      f: editing
        ? { name: editing.name, group: editing.group || "", type: editing.type || "Dịch vụ", price: String(editing.price ?? ""), unit: editing.unit || "Răng", mins: String(editing.mins ?? 30) }
        : { name: "", group: group || allGroupNames[0] || "", type: "Dịch vụ", price: "", unit: "Răng", mins: "30" },
    });
  };
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {list.length === 0 ? (
                  <tr><td colSpan={6} className="px-4 py-12 text-center text-slate-400">Không có dịch vụ phù hợp.</td></tr>
                ) : list.map((s, idx) => {
                  ...
                  return (
                    <tr key={s.id} className={`hover:bg-slate-50 transition ${active ? "" : "opacity-60"}`}>
                      ...
                      <td className="px-4 py-3">
                        <div className="font-mono text-[11px] text-emerald-700/80">{s._code}</div>
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <Field label="Thời lượng (phút)">
              <input type="number" className={inputCls} value={modal.f.mins} onChange={(e) => setModal({ ...modal, f: { ...modal.f, mins: e.target.value } })} />
            </Field>
          </div>
        </Modal>
      )}
